...
exports.createItem = async (req, res) => {
  ...
  try {
    // ✅ Let MySQL auto-increment handle item_id
    const [result] = await db.execute(
      `INSERT INTO items (item_name, type, units, kg, grams, litres, status_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [name, type, units, kg, grams, litres, status]
    );

    res.status(201).json({
      success: true,
      data: {
        item_id: result.insertId,   // auto-generated by MySQL
        name,
        type,
        units,
        kg,
